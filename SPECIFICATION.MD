...
4.4 `Storage`: SQLite (embedded) for offline saves, PostgreSQL/Azure Cosmos optional adapters for cloud sync. JSON5 save export for portability. Asset metadata stored as JSON5 + binary atlases for 8x8–512x512 textures.
...
5.3 `Engine Loop`: Fixed timestep (default 100ms) with sub-step resolution for prestige bursts; `ITickConsumer` interface for plug-ins.
...
8.5 `Graphics Settings`: In-game menu surfaces render rate (30/60/120/240 fps), resolution scale, sprite smoothing, particle density, and GPU tier selection with safe defaults for low-end hardware.
...
9.3 `Caching`: Deterministic memoization for formula-heavy calculations; configurable LRU caches with telemetry hooks.
...
10.1 `Version Control`: GitHub feature branches (`feature/<id>-<slug>`). PR template requires checklist: tests, TODO updates, spec impact.
...
10.4 `Testing Mandate`: Every feature branch must include a dedicated automated test suite (unit + integration + GPU/render snapshots as applicable) before merge; PRs without tests are blocked by policy checks.
...
11.5 `Memory Ledger`:
```
Doc: docs/MEMORY-LEDGER.md
Entry Format (Machine-first):
  - `EntryId`: Stable identifier (`MEM-<UTC compact>`)
  - `Timestamp`: ISO 8601 UTC
  - `Scope`: Semicolon-delimited topics/modules touched
  - `Summary`: <=280 character compressed description of the exchange
  - `Decisions`: Bullet list, each `DECISION:<text>`
  - `Actions`: Bullet list, each `ACTION:<text>` describing code/tests/docs updates
Entries ordered newest-first, separated by `---` line.
Purpose: Persist AI/operator context across sessions while remaining diff-friendly and easy for tooling to parse.
```
